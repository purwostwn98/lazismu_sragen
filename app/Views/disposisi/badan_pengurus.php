...
<div class="d-flex align-items-center justify-content-between flex-wrap gap-2 mb-4">
  <div>
    <h4 class="mb-1"><?= esc($title ?? 'Disposisi Badan Pengurus') ?></h4>
    <p class="text-body-secondary mb-0">
      Total <?= count($rows) ?> ajuan bernilai lebih dari Rp 5.000.000 sudah ditinjau Manager, menunggu tinjauan Badan Pengurus.
    </p>
  </div>
</div>
      ...
      <tbody>
        <?php foreach ($rows as $i => $a): ?>
          <?php
            ...
            $detailUrl   = base_url('disposisi/badan-pengurus/' . $a['nomor_ajuan']);
          ...
          <tr>
            ...
            <td>
              ...
              <?php elseif ((int) $manager['rekomendasi'] === 1): ?>
                <span class="badge bg-label-success">Disetujui</span>
                <span class="text-body-secondary ms-1">Rp <?= number_format((float) $manager['nominal_rekomendasi'], 0, ',', '.') ?></span>
              <?php else: ?>
