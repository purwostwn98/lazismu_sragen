...
<div class="d-flex align-items-center justify-content-between flex-wrap gap-2 mb-4">
  <div>
    <h4 class="mb-1"><?= esc($title ?? 'Disposisi Manager') ?></h4>
    <p class="text-body-secondary mb-0">Total <?= count($rows) ?> ajuan sudah ditinjau Kepala Divisi Program, menunggu tinjauan Manager.</p>
  </div>
</div>
    ...
    <table class="table table-hover table-sm datatable w-100">
      <thead>
        <tr>
          ...
          <th>Status</th>
          <th>Rekomendasi Kepala Divisi Program</th>
      ...
      <tbody>
        <?php foreach ($rows as $i => $a): ?>
          <?php
            ...
            $detailUrl   = base_url('disposisi/manager/' . $a['nomor_ajuan']);
            $kadiv       = $a['kadiv'] ?? null;
          ?>
          <tr>
            <td><?= $i + 1 ?></td>
            ...
            <td>
              ...
              <?php elseif ((int) $kadiv['rekomendasi'] === 1): ?>
                <span class="badge bg-label-success">Disetujui</span>
                <span class="text-body-secondary ms-1">Rp <?= number_format((float) $kadiv['nominal_rekomendasi'], 0, ',', '.') ?></span>
              <?php else: ?>
